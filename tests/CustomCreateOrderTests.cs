...
[NonParallelizable]
[TestFixture]
public class CustomCreateOrderTests : BaseSetupWithOAuth
{
    ...
    [Test, Order(1)]
    public async Task CustomCheckout_CreateOrder_Successful()
    {
        string url = "/payments/v2/sdk/order";

        OAuthSetup();

        var createOrderRequest = CreateSdkOrderRequest.CustomCheckoutBuilder()
            .SetAmount(100)
        ...
        var actual = await customCheckoutClient.CreateSdkOrder(createOrderRequest);
